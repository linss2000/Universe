...
import { actions as BAPActions } from "../reducers/bapreducer";
...
export class BAP extends Component {
  static propTypes = {
  ...
  _onFilterChange() {
    ...
    const size = this.props.BAPState.items.length;

    let filteredItems = [];

    for (var index = 0; index < size; index++) {
      const { hv_cadet_name } = this.props.BAPState.items[index];

      if (hv_cadet_name.toLowerCase().indexOf(filterBy) !== -1) {
        filteredItems.push(this.props.BAPState.items[index]);
      }
    ...
    this.props.makeRowEditable({
      type: BAPTypes.MAKE_ROW_EDITABLE,
      payload: {
        rowID: -1
      }
    });
          ...
          <Nav tabs className="m-0 p-0">
            {!this.state.inDetailsTab && (
              <NavItem>
                ...
                >
                  <i className="fa fa-home" /> Staff and Budget
                </NavLink>
              </NavItem>
            )}
            {!this.state.inDetailsTab && (
              <NavItem>
                ...
                >
                  <i className="fa fa-podcast" /> Purchase Orders
                </NavLink>
              </NavItem>
            )}            
          </Nav>
...
export default connect(mapStateToProps, mapDispatchToProps)(BAP);
