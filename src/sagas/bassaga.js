import {
  ...
  const basApi = {
    ...
    //alert("in Cadets")
      ...
      return fetch("http://hvs.selfip.net:3003/getBudgets/", {
  ...
  function* getBudgetTables(cname) {
    ...
    try {
      ...
      //yield put({ type: basTypes.LOGIN_REQUEST, isLoading: false })
      const resultObj = yield call(basApi.getBudgetTables, cname);
  
      debugger;
      if (resultObj.response && !resultObj.response.ok) {
        debugger;
        yield put({
          type: basTypes.MESSAGE,
      ...
      debugger;
      yield put({ type: basTypes.MESSAGE, message: {val:-1, msg:e} });
    } finally {
      debugger;
      if (yield cancelled())
        yield put({ type: basTypes.MESSAGE, message: {val: -1, msg:"Task Cancelled" }});
    }
  }
  ...
  export function* handleRequest(action) {
    ...
    try {
      switch (action.type) {
        ...
        case basTypes.FETCH_TABLES_REQUEST: {
      ...
      yield put({ type: basTypes.LOGIN_FAILURE, error: e });
    }
  }
